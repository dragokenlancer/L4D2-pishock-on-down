...
def load_config():
    ...
    with open(CONFIG_FILE, encoding="utf-8") as f:
        raw = f.read()
    # Strip _comment keys so json.loads doesn't choke (they're valid JSON, but let's be safe)
...
def send_shock(cfg, intensity: int, duration_sec: float):
    ...
    if cfg.get("dry_run", False):
        print(f"  [DRY RUN] Would shock via {provider}: intensity={intensity}%  duration={duration_sec:.1f}s  (no API call made)")
        return

    print(f"  → Shocking: provider={provider}  intensity={intensity}%  duration={duration_sec:.1f}s")
    try:
        if provider == "pishock":
            code, body = send_pishock(cfg, intensity, duration_sec)
        elif provider == "openshock":
            code, body = send_openshock(cfg, intensity, duration_sec)
        else:
            print(f"  [!] Unknown provider '{provider}' — set to 'pishock' or 'openshock' in config.json")
            return
        print(f"  ← HTTP {code}: {body[:120]}")
...
def handle_event(event_str: str, cfg: dict, last_shock_time: list) -> list:
    ...
    if etype == "LOADED":
        ...
        print(f"\n[Companion] Player downed! #{down_num}  diff={diff_idx}  raw intensity={intensity}%  duration={duration:.1f}s")
...
def tail_log(log_path: str, cfg: dict):
    ...
    with open(log_path, "r", encoding="utf-8", errors="replace") as f:
        f.seek(0, 2)   # jump to end — ignore existing content
        while True:
            line = f.readline()
            if not line:
                time.sleep(0.05)
